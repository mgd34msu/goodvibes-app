// wake-host.ts, this webview as a wake-word surface. Adapted from
// goodvibes-webui src/lib/voice/wake-host.ts (surface 'app' rather than
// 'webui', model bytes loaded through this app's own wake-models.ts loader,
// otherwise the same host: the SDK owns everything that is identical on
// every surface, the front end, the scoring rules, the pre-roll, the
// silence and ceiling policy, the engine reset after a command, the
// restart/latch policy (`WakeListener` is all of that). What a host owns is
// four things, and this file is those four:
//
//   1. a device      , the shared webview capture opener, through the arbiter,
//   2. an inference runtime, onnxruntime-web sessions over locally-served models,
...
//
// DISABLED MEANS THE DEVICE IS NEVER TOUCHED. `voice.wake.surfaces.app`
// defaults to false, and while it is false (or `voice.wake.enabled` is off,
// or a row blocks) this host loads no model, creates no session and calls no
// `getUserMedia`, so no permission prompt appears. `.active` from the
// resolver is the ONLY thing consulted for that decision; nothing here
// re-derives it.
//
// A module singleton, for the same reason the TTS engine
// (views/chat/voice.ts) is one: there is one microphone and one webview.
// `createWakeHost` is exported beside it so a test drives the real engine
// and the real listener with a stubbed inference session, rather than
// testing a second implementation of this logic.
import {
...
import { micArbiter, type MicArbiter } from "./mic-arbiter.ts";
import { playWakeChime } from "./wake-chime.ts";
import { loadWakeModel, openWakeModelCache, WakeModelError, type WakeModelComponent } from "./wake-models.ts";
import { loadWakeRuntime, type WakeBrowserBackend } from "./wake-runtime.ts";
import { CONFIG_SCHEMA_SNAPSHOT } from "../../views/settings/config-schema.generated.ts";

/**
 * The model id the daemon actually serves to this app. Read from this repo's
 * generated schema snapshot (the drift-checked copy of the SDK's own row)
 * rather than spelled here, so it cannot fall out of step with the pinned
 * model.
 */
export const PINNED_WAKE_MODEL_ID: string = (() => {
  const row = CONFIG_SCHEMA_SNAPSHOT.find((entry) => entry.key === "voice.wake.models");
  const value = typeof row?.default === "string" ? row.default.split(",")[0]?.trim() : "";
  return value && value.length > 0 ? value : "hey_goodvibes";
})();

/** What this app is doing about wake detection, for an indicator that must not lie. */
...
export interface WakeModelSet {
  ...
  /**
   * The speech gate, present only when `voice.wake.vadThreshold` asks for one
   * AND its artifact is provisioned. Absent means every frame is scored,
   * which is what the shipped default of 0 asks for, never a gate that
   * failed to load, because that is a startup blocker from
...
export interface WakeHostDeps {
  ...
  /**
   * Builds the speex suppression stage. The SDK's listener wraps the opener
   * and defaults to the embedded WebAssembly filter; injected here so a test
   * can check that captured frames REACH a stage without compiling one,
   * while the filter's own numbers stay the SDK's to assert.
   */
  ...
  /** Plays the activation sound; false when this webview could not make a sound. */
...
export class WakeHost {
  ...
  /**
   * Register where transcripts go. Returns the unregister function. The host
   * keeps working with no sink registered, it still listens, chimes and
   * transcribes, and reports the transcript in its own state, so a wake is
   * never silently lost because no view happened to be mounted.
   */
   ...
   * configuration is a no-op, so this can be called from a render path on
   * every `config.get` refetch.
   */
  applySettings(settings: WakeRuntimeSettings | null): Promise<void> {
    return this.#run(async () => {
      ...
      if (unchanged) {
        // Indicator and the written blocker/limitation lists can change
        // without the runtime needing a rebuild.
      ...
      if (!settings.active) {
        // The one place the "never open a device while disabled" rule is
        // enforced: nothing below this line runs, so no model is fetched and
        ...
        this.#patch({
          ...
          // A user who switched this on for this install and got a blocker
          // still gets the indicator, carrying the reason, the alternative is
          // a feature that reads as enabled and shows nothing anywhere.
  ...
  async #startListening(settings: WakeRuntimeSettings): Promise<void> {
    ...
    const listener = new WakeListener({
      ...
      createEngine: () => Promise.resolve(new WakeWordEngine({
        ...
        // Required in practice: the engine imports no logger of its own, so a
        // model that misbehaves is only ever visible through this sink.
        warn: this.#deps.warn,
      })),
      handlers: {
        ...
        onWake: ({ activationSound }) => {
          ...
          if (!played) this.#deps.warn("wake confirmed but this webview could not play the activation chime");
  ...
  #onListenerState(state: WakeListenerState): void {
    // The listener's own phases map onto this host's, EXCEPT while a
    // transcript is in flight (the listener is back to 'listening' by then,
    // but the user is waiting on words) and while suspended (the host
  ...
  async #onUtterance(utterance: CapturedUtterance, detection: WakeDetection): Promise<void> {
    if (utterance.silent) {
      ...
      // provider bills a request to transcribe a room, so it is reported
      // instead.
...
function describeInactive(settings: WakeRuntimeSettings): WakeHostState["refusal"] {
  ...
  if (!settings.surfaceEnabled) {
    return {
      ...
      detail: "voice.wake.surfaces.app is off, so this app does not listen and never asks for the microphone. "
        + "Turn it on in Voice settings to opt this install in.",
    };
  }
...
// ─── This app's default dependencies ─────────────────────────────────────────

/**
 * Load the classifier and the embedding backbone (and the speech gate when
 * asked for), verify both, and bring an inference runtime up over them.
 *
 * Only the PINNED classifier is servable to this app: `voice.wake.model`
 * reads the managed artifacts, and a custom model in
 * `voice.wake.customModelDir` lives on the daemon host's disk with no route
 * to fetch it. Extra ids are therefore reported as a limitation rather than
 * silently relabelled onto the pinned model, a detector that says it is
 * running your model and is running a different one is worse than one that
 * says it cannot.
 */
export function createAppModelSetLoader(deps: {
  readonly readChunk: (input: { component: WakeModelComponent; offset: number }) => Promise<{
  ...
  return async (settings) => {
    const cache = await openWakeModelCache();
    const loaderDeps = {
    ...
    const [embeddingLoaded, classifierLoaded, vadLoaded] = await Promise.all([
      loadWakeModel("embedding", loaderDeps),
      loadWakeModel("classifier", loaderDeps),
      // Fetched, cached and CHECKSUM-VERIFIED exactly like the other two, a
      // gate assembled from a truncated transfer would screen frames by
      // accident.
      wantsGate ? loadWakeModel("vad", loaderDeps) : Promise.resolve(null),
    ]);

    const runtime = await loadWakeRuntime(settings.browserBackend);
    const embedding = await runtime.createSession(embeddingLoaded.bytes);
    const classifier = await runtime.createSession(classifierLoaded.bytes);
    const vadSession = vadLoaded === null ? null : await runtime.createSession(vadLoaded.bytes);

    const limitations: string[] = [];
    const unservable = settings.modelIds.filter((id) => id !== PINNED_WAKE_MODEL_ID);
    if (unservable.length > 0) {
      limitations.push(
        `voice.wake.models names ${unservable.join(", ")}, which this app cannot load: the daemon serves the `
        + `pinned "${PINNED_WAKE_MODEL_ID}" classifier, and a custom model lives on the daemon host's disk with no `
        + "route to fetch it here. Only the pinned model is being scored in this app.",
      );
    }
    ...
    if (models.length === 0) {
      limitations.push(
        `voice.wake.models does not include "${PINNED_WAKE_MODEL_ID}", so this app scores nothing. Capture is `
        + "still available for dictation.",
      );
    }
...
/**
 * Create a host over explicit dependencies. Used by the singleton below and
 * by tests, which supply a fake capture stream and a stub inference session
 * so the REAL engine and the REAL listener are what gets exercised.
 */
export function createWakeHost(deps: WakeHostDeps): WakeHost {
  return new WakeHost(deps);
}

/** Warnings go to the console: this app has no log file the webview can write
 * to, and silence is worse. */
...
/**
 * This app's wake host.
 *
 * Its daemon-facing dependencies are installed by `installWakeHostDaemonDeps`
 * rather than imported here, so this module does not pull `lib/gv.ts` (and
 * with it the whole SDK client surface) into the import graph of anything
 * that only wants the capture primitive.
 */
...
/** Wire the singleton's daemon-facing calls. Called once, from useWake.ts. */
