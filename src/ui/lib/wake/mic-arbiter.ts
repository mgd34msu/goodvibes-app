...
// Ported verbatim from goodvibes-webui src/lib/voice/mic-arbiter.ts.
//
// Wake detection holds the microphone continuously for as long as the user
// has it on. Push-to-talk dictation wants the same device, on a keypress,
// right now. Without an arbiter both call `getUserMedia` and the tab ends up
// with two live streams on one input: on some platforms the second open
// fails, on others it succeeds and the two consumers fight over gain and
// device state. Either way the user sees a microphone that "sometimes does
// not work".
//
...
// before the dictation stream is opened, and restarted when dictation
// finishes, including when it fails. The alternative (dictation refusing
// while wake holds the device) makes the headline affordance stop working
// the moment the always-on feature is switched on, which is worse than a
// two-second gap in detection.
//
// Both consumers go through {@link MicArbiter.openCapture}, and it REFUSES a
// second concurrent open with `device-unavailable` rather than allowing it.
// That refusal is the invariant this file exists for: any future third
// consumer is caught by it instead of quietly doubling the streams.
//
// This app's existing MicButton/useVoiceInput dictation path
// (views/chat/voice.ts) predates the wake-word port and still opens its own
// MediaRecorder stream directly, not through this arbiter, so the two
// consumers can still collide until that path adopts mic-arbiter.ts too;
// tracked as a known gap rather than folded into this port silently.
...
import { createBrowserCaptureOpener } from "./capture.ts";
...
export class MicArbiter {
  ...
  /**
   * Register the always-on consumer so it can be stood down for a press.
   * Returns the unregister function; the wake host calls it when it stops
   * for good.
   */
  ...
  /**
   * The opener both consumers use. Refuses a second concurrent stream instead
   * of opening one, the bug this class exists to prevent.
   */
  ...
  /**
   * Take the device for a deliberate press: stand wake detection down, wait
   * for the device to actually be free, and return the lease that resumes it.
   *
   * Overlapping calls are serialised, a second press waits for the first
   * lease to be released rather than suspending an already-suspended
   * listener and resuming it underneath the first holder.
   */
...
/**
 * The tab's single arbiter. A module singleton for the same reason the TTS
 * engine is one: there is exactly one microphone and exactly one webview, so
 * a per-component instance would recreate the very race this arbitrates.
 */
