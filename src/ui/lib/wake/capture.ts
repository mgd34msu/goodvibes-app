// capture.ts, the ONE webview microphone path, shared by wake-word detection
// and (once mic-arbiter.ts is adopted by dictation) push-to-talk. Ported
// verbatim from goodvibes-webui src/lib/voice/capture.ts: this is the web
// UI's implementation of the SDK's `AudioCaptureOpener` (the platform/voice
// capture subpath imported below), and the webview's WebKitGTK surface is
// the same secure-context browser environment that file already targets,
// nothing here is app-specific.
//
// THE FRAME CONTRACT IS LOAD-BEARING AND ITS FAILURES ARE SILENT
//
// `onFrame` must deliver exactly `request.frameSamples` samples (1280 = 80 ms
// at 16 kHz) of mono audio carrying int16 MAGNITUDES as floats
// (-32768..32767), not the -1..1 range Web Audio hands out. That is the scale
// openWakeWord trained the shipped classifier against: feeding it normalised
// audio produces scores that never reach any threshold, which looks exactly
// like a microphone that is not picking anything up. So every sample is
// multiplied by 32768 on the way through, and the worklet's render quanta are
// re-cut by the SDK's `AudioFrameSlicer` rather than assumed to be
// frame-sized (they are 128 samples).
//
// SAMPLE RATE IS ASKED FOR AND THEN VERIFIED
//
// `getUserMedia` is asked for 16 kHz mono and the AudioContext is constructed
// at 16 kHz so the graph resamples deterministically, but a browser may
// ignore either. `audioContext.sampleRate` is READ, and when it is not
// 16 kHz the frame pump resamples to 16 kHz itself rather than handing the
// detector audio at the wrong rate (which shifts the whole front end off the
// framing the classifier was trained at, and again fails silently).
//
// Every failure is an `AudioCaptureError` with a named reason, so the surface
// can render the specific honest state, the insecure-context pointer, the
// blocked permission, the device that is already held, instead of one dead
// button.
...
/**
 * Whether the microphone can be used here at all, decided before any capture
 * is attempted. Local to this module (the composer's dictation path keeps its
 * own three-state MicSupport in views/chat/voice.ts, a different capture
 * stack): the wake host and its mic-arbiter never consult that one.
 */
export type MicSupport = "ok" | "insecure-context" | "unsupported";

// ─── The minimal browser surface this module touches ─────────────────────────
// Structural interfaces rather than the DOM lib types: a unit test drives a
// fake AudioContext and a fake mediaDevices, and a fake only has to satisfy
...
export function browserCaptureEnv(): CaptureEnv {
  ...
  const urlApi = w.URL;
  return {
    // The webview serves from 127.0.0.1 (a secure context), same as
    // localhost in a regular browser.
 ...
 * state) rather than a bare 'unsupported'.
 */
...
export class CaptureFramePump {
  ...
  /**
   * Linear interpolation to 16 kHz, continuous across buffers: the tail the
   * next output sample interpolates from is CARRIED, so a resampled stream
   * has no discontinuity at every buffer boundary (which would read as a
   * periodic click to the mel front end).
   */
 ...
 * the "carries int16 magnitudes" claim stays true for a hot input that Web
 * Audio allows to exceed 1.0.
 */
 ...
 * build-output path between the two. It batches render quanta before posting
 * so the main thread receives ~43 messages a second rather than ~344; the
 * SDK's slicer re-cuts whatever arrives, so the batch size is free to be a
 * performance choice.
 */
...
/**
 * Build the webview's `AudioCaptureOpener`.
 *
 * `request.backend` is ignored: it names a host recorder subprocess and a
 * webview tab has none, which the `voice.wake.captureCommand` row says in
 * writing.
 *
 * `request.noiseSuppression: 'speex'` reaching THIS opener is refused, and
 * that is not a statement about the platform: the speexdsp filter is a
 * WebAssembly module the SDK carries and it runs perfectly well in a tab, one
 * layer up, inside `createNoiseSuppressingOpener`, which WakeListener and
 * PushToTalkSession apply and which asks this opener for raw frames. So a
 * `speex` request arriving here means the wrapper was bypassed, and passing
 * it through would hand back unfiltered audio under a filter the user
 * configured.
 */
export function createBrowserCaptureOpener(env: CaptureEnv = browserCaptureEnv()): AudioCaptureOpener {
  return async (request: AudioCaptureRequest, handlers: AudioCaptureHandlers): Promise<AudioCaptureStream> => {
    ...
    if (support === "unsupported" || !env.mediaDevices || !env.createAudioContext) {
      throw new AudioCaptureError("unsupported", "This webview cannot capture the microphone.");
    }
    ...
    try {
      ...
      releaseDevice();
      throw new AudioCaptureError("unsupported", "This webview could not open an audio graph for the microphone.");
    }
 ...
 * blob: scripts, or a browser without the API. The fallback is deprecated,
 * not broken, and a working microphone beats a correct-but-dead one.
 */
async function attachTap(
  ...
  throw new AudioCaptureError("unsupported", "This webview has no way to read samples from the microphone.");
}
