// wake-chime.ts, the sound a confirmed wake makes. Ported verbatim from
// goodvibes-webui src/lib/voice/wake-chime.ts (no app-specific logic here).
//
// Audible confirmation is how a user knows the microphone acted; a silent
// wake is the behaviour people distrust, which is why `voice.wake.
// activationSound` defaults to "chime" rather than "none". A webview cannot
// read `voice.wake.activationSoundPath` off the user's disk, so the settings
// resolver has already downgraded a "custom" choice to "chime" and recorded
// that as a limitation, this module only ever plays the built-in one.
//
// Synthesised with two short oscillator notes rather than shipped as an
// audio asset: it is 200 ms of two sine tones, and a bundled file would be a
// network fetch in front of the one piece of feedback that has to be
// immediate.
...
/**
 * Play the activation chime. Returns true when it was actually scheduled, so
 * the wake host can report "no sound available here" instead of assuming one
 * played.
 *
 * A fresh context per chime, closed on the way out: holding one open for the
 * life of the tab keeps an audio device warm for a sound that plays for a
 * fifth of a second, and a tab that has never had a user gesture cannot
 * resume a stale one anyway.
 */
