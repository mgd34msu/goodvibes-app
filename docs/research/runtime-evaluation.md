...
**Linux support status (the deciding factor), verified and nuanced:**
...
- **`Bun.WebView`** (built-in, experimental): **headless only**, for automation/scraping; `headless:false` not implemented. Not a windowing option; ruled out.
...
1. **Zero-friction dev loop on Arch**: one runtime, one language, via `bun install electrobun`, `npx electrobun init`, dev/build from the same CLI; deps are just `gtk3` + `webkit2gtk-4.1` from pacman. No Rust toolchain, no sidecar binary matrix.
